...
DVKBuffer* DVKBuffer::CreateBuffer(VulkanDeviceMemoryManager manager,std::shared_ptr<VulkanDevice> vulkanDevice,VkBufferUsageFlags usageFlags,VkMemoryPropertyFlags memoryPropertyFlags,VkDeviceSize size,void* data)
{
	DVKBuffer* dvkBuffer = new DVKBuffer();
	dvkBuffer->device = vulkanDevice->GetInstanceHandle();
	VkDevice vkDevice = vulkanDevice->GetInstanceHandle();
	...
	vkGetBufferMemoryRequirements(vkDevice,dvkBuffer->buffer,&memReqs);
	manager.GetMemoryTypeFromProperties(memReqs.memoryTypeBits,memoryPropertyFlags,&memoryTypeIndex);
	memAlloc.allocationSize = memReqs.size;
	memAlloc.memoryTypeIndex = memoryTypeIndex;

...
VulkanDrawable::VulkanDrawable(VulkanRenderer * parent)
{
	...
	//创建了两个用来同步的信号量,信号量这种同步机制可以跨队列或者单一队列中的粗粒度指令缓存中执行
...
void VulkanDrawable::initViewports(VkCommandBuffer * cmd)
{
	...
	viewport.x = 0;
	viewport.y = 0;
	//只有流水线启用了动态状态VK_DYNAMIC_STATE_VIEWPORT,这个接口设置才有效
...
void VulkanDrawable::initScissors(VkCommandBuffer * cmd)
{
	...
	scissor.offset.x = 0;
	scissor.offset.y = 0;
	//只有流水线状态对象创建的时候启用了动态状态VK_DYNAMIC_STATE_SCISSOR,这个接口设置才有效

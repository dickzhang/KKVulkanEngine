...
IndirectDrawDemo::IndirectDrawDemo(int32 width,int32 height,const char* title,const std::vector<std::string>& cmdLine) :
	ModuleBase(width,height,title,cmdLine)
{
}

IndirectDrawDemo::~IndirectDrawDemo()
{

}
bool IndirectDrawDemo::Init()
{
	ModuleBase::Setup();
	ModuleBase::Prepare();
...
void IndirectDrawDemo::Loop(float time,float delta)
{
	if(!m_Ready)
	{
		return;
...
void IndirectDrawDemo::ExtentAABBPoints(Vector4* aabbPoints,const Vector4& center,const Vector4& extent)
{
...
void IndirectDrawDemo::ComputeFrustumFromProjection(Frustum& out,const Matrix4x4& projection)
{
...
Vector4 IndirectDrawDemo::VectorSelect(const Vector4& v1,const Vector4& v2,const Vector4& control)
{
	Vector4 result;
...
void IndirectDrawDemo::CreateFrustumPointsFromCascadeInterval(float cascadeIntervalBegin,float cascadeIntervalEnd,const Matrix4x4& projection,Vector4* cornerPointsWorld)
{
...
Vector4 IndirectDrawDemo::VectorMin(const Vector4& a,const Vector4& b)
{
	Vector4 result;
...
Vector4 IndirectDrawDemo::VectorMax(const Vector4& a,const Vector4& b)
{
	Vector4 result;
...
void IndirectDrawDemo::ComputeNearAndFar(float& nearPlane,float& farPlane,const Vector4& lightCameraOrthographicMin,const Vector4& lightCameraOrthographicMax,Vector4* pointsInCameraView)
{
...
void IndirectDrawDemo::UpdateCascade()
{
...
void IndirectDrawDemo::Draw(float time,float delta)
{
...
bool IndirectDrawDemo::UpdateUI(float time,float delta)
{
	m_GUI->StartFrame();
...
void IndirectDrawDemo::CreateRenderTarget()
{
...
void IndirectDrawDemo::DestroyRenderTarget()
{
	delete m_ShadowMap;
	delete m_ShadowRTT;
}

void IndirectDrawDemo::LoadAssets()
{
	...
	// ground model
	m_GroundModel = DVKModel::LoadFromFile(
	...
	m_GroundShader = DVKShader::Create(
		m_VulkanDevice,
		true,
	...
	m_GroundMaterial = DVKMaterial::Create(
		m_VulkanDevice,
		m_RenderPass,
		m_PipelineCache,
		m_GroundShader
	...
	// plants model
	m_PlantsModel = DVKModel::LoadFromFile(
	...
	m_PlantsShader = DVKShader::Create(
		m_VulkanDevice,
		true,
	...
	m_PlantsMaterial = DVKMaterial::Create(
		m_VulkanDevice,
		m_RenderPass,
		m_PipelineCache,
		m_PlantsShader
	...
	for(int32 i = 0; i<m_PlantsModel->meshes.size(); ++i)
	{
		...
		{
			DVKPrimitive* primitive = m_PlantsModel->meshes[i]->primitives[p];
	...
	// 创建上传Buffer
	{
		DVKBuffer* instanceStagingBuffer = DVKBuffer::CreateBuffer(
			m_VulkanDevice,
		...
		DVKBuffer* vertStagingBuffer = DVKBuffer::CreateBuffer(
			m_VulkanDevice,
			VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
			VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
		...
		DVKBuffer* indexStagingBuffer = DVKBuffer::CreateBuffer(
			m_VulkanDevice,
			VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
			VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
		...
		DVKBuffer* cmdStagingBuffer = DVKBuffer::CreateBuffer(
			m_VulkanDevice,
			VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
			VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
		...
		m_IndirectVertexBuffer = DVKBuffer::CreateBuffer(
			m_VulkanDevice,
		...
		m_IndirectInstanceBuffer = DVKBuffer::CreateBuffer(
			m_VulkanDevice,
			VK_BUFFER_USAGE_VERTEX_BUFFER_BIT|VK_BUFFER_USAGE_TRANSFER_DST_BIT,
			VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
		...
		m_IndirectIndexBuffer = DVKBuffer::CreateBuffer(
			m_VulkanDevice,
		...
		m_IndirectCmdBuffer = DVKBuffer::CreateBuffer(
			m_VulkanDevice,
...
void IndirectDrawDemo::DestroyAssets()
{
...
void IndirectDrawDemo::RenderPlantsDepth(VkCommandBuffer commandBuffer)
{
...
void IndirectDrawDemo::RenderGround(VkCommandBuffer commandBuffer)
{
...
void IndirectDrawDemo::RenderPlants(VkCommandBuffer commandBuffer)
{
...
void IndirectDrawDemo::BeginMainPass(VkCommandBuffer commandBuffer,int32 backBufferIndex)
{
...
void IndirectDrawDemo::RenderDebug(VkCommandBuffer commandBuffer)
{
	VkViewport viewport = { };
...
void IndirectDrawDemo::SetupCommandBuffers(int32 backBufferIndex)
{
...
void IndirectDrawDemo::InitParmas()
{
...
void IndirectDrawDemo::CreateGUI()
{
	m_GUI = new ImageGUIContext();
	m_GUI->Init("Assets/fonts/Ubuntu-Regular.ttf");
}

void IndirectDrawDemo::DestroyGUI()
{
	m_GUI->Destroy();
	delete m_GUI;
}
#include "ComputeParticlesDemo.h"
...
ComputeParticlesDemo::ComputeParticlesDemo(int32 width,int32 height,const char* title,const std::vector<std::string>& cmdLine) :
	ModuleBase(width,height,title,cmdLine)
{
}

ComputeParticlesDemo::~ComputeParticlesDemo()
{

}

bool ComputeParticlesDemo::Init()
{
	ModuleBase::Setup();
	ModuleBase::Prepare();
...
void ComputeParticlesDemo::Exist()
{
	DestroyAssets();
	DestroyGUI();
	ModuleBase::Release();
...
void ComputeParticlesDemo::Loop(float time,float delta)
{
	if(!m_Ready)
	{
		return;
...
void ComputeParticlesDemo::Draw(float time,float delta)
{
...
bool ComputeParticlesDemo::UpdateUI(float time,float delta)
{
	m_GUI->StartFrame();
...
void ComputeParticlesDemo::LoadAssets()
{
...

	{
		...
		m_ParticleBuffer = DVKBuffer::CreateBuffer(
			m_VulkanDevice,
	...
	m_GradientTexture = DVKTexture::Create2D(
	...
	m_DiffuseTexture = DVKTexture::Create2D(
...
void ComputeParticlesDemo::DestroyAssets()
{
...
void ComputeParticlesDemo::SetupComputeCommand()
{
...
void ComputeParticlesDemo::SetupGfxCommand(int32 backBufferIndex)
{
	VkViewport viewport = { };
	viewport.x = 0;
...
void ComputeParticlesDemo::InitParmas()
{
...
void ComputeParticlesDemo::CreateGUI()
{
	m_GUI = new ImageGUIContext();
	m_GUI->Init("Assets/fonts/Ubuntu-Regular.ttf");
}

void ComputeParticlesDemo::DestroyGUI()
{
	m_GUI->Destroy();
	delete m_GUI;
}
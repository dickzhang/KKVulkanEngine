...
{
	return std::make_shared<DynamicUniformBuffer>(1400,900,"DynamicUniformBuffer",cmdLine);
}

DynamicUniformBuffer::DynamicUniformBuffer(int32 width,int32 height,const char* title,const std::vector<std::string>& cmdLine) :
...
bool DynamicUniformBuffer::Init()
{
	ModuleBase::Setup();
	ModuleBase::Prepare();
...
void DynamicUniformBuffer::Exist()
{
	ModuleBase::Release();
	DestroyAssets();
	DestroyGUI();
...
void DynamicUniformBuffer::Loop(float time,float delta)
{
	if(!m_Ready)
	{
		return;
...
void DynamicUniformBuffer::Draw(float time,float delta)
{
...
void DynamicUniformBuffer::CreateGUI()
{
	m_GUI = new ImageGUIContext();
	m_GUI->Init("assets/fonts/Ubuntu-Regular.ttf");
}

bool DynamicUniformBuffer::UpdateUI(float time,float delta)
{
	m_GUI->StartFrame();
...
void DynamicUniformBuffer::DestroyGUI()
{
	m_GUI->Destroy();
	delete m_GUI;
}
void DynamicUniformBuffer::LoadAssets()
{
...
void DynamicUniformBuffer::DestroyAssets()
{
	delete m_Model;
}

void DynamicUniformBuffer::UpdateUniformBuffers(float time,float delta)
{
	uint32 alignment = m_VulkanDevice->GetLimits().minUniformBufferOffsetAlignment;
...
void DynamicUniformBuffer::SetupCommandBuffers()
{
...
void DynamicUniformBuffer::CreateUniformBuffers()
{
	...
	m_ColorBuffer = DVKBuffer::CreateBuffer(
		m_VulkanDevice,
		VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
		VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
		m_ColorDatas.size(),
...
void DynamicUniformBuffer::DestroyUniformBuffers()
{
...
void DynamicUniformBuffer::CreateDescriptorSetLayout()
{
...
void DynamicUniformBuffer::DestroyDescriptorSetLayout()
{
...
void DynamicUniformBuffer::CreateDescriptorSet()
{
...
void DynamicUniformBuffer::CreatePipelines()
{
	...
	pipelineInfo.vertShaderModule = LoadSPIPVShader(m_Device,"Assets/Shaders/10_DynamicUniformBuffer/obj.vert.spv");
	pipelineInfo.fragShaderModule = LoadSPIPVShader(m_Device,"Assets/Shaders/10_DynamicUniformBuffer/obj.frag.spv");
...
void DynamicUniformBuffer::DestroyPipelines()
{
	delete m_Pipeline;
	m_Pipeline = nullptr;
}
